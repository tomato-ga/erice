...
import { ItemType, DMMItemProps } from '@/types/dmmtypes'
...
// item: 商品情報 (T )
const DMMItemCard = <T extends DMMItemProps>({ item, itemType, from }: { item: T; itemType: ItemType; from: string }) => {
	return (
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
				{items.slice(0, displayCount).map((item) => (
						<div key={item.content_id}>
							<DMMItemCard item={item} itemType={itemType} from={from} />
						</div>
				))}
			</div>
	)
