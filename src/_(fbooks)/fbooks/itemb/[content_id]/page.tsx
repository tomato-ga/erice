// src/app/(fbooks)/fbooks/itemb/[content_id]/page.tsx

import { FbooksItem, FbooksItemType, FbooksKobetuItem } from '@/_types_fbooks/fbookstype'
...
import SeriesTimelinePage from '@/app/components/fbookscomponents/kobetu/SeriesTimeline'
import {
	generateDoujinKobetuItemStructuredData,
	generateFbooksBreadcrumbList,
	generateFbooksKobetuItemStructuredData,
...
import FanzaADBannerKobetu from '@/app/components/fbookscomponents/fanzaADBannerKobetu'
// ItemDetailsTableコンポーネント
import React from 'react'

const ItemDetailsTable: React.FC<{ item: FbooksKobetuItem }> = ({ item }) => {
	const details = [
		{ label: 'タイトル', value: item.title, icon: '🎬' },
		{
			label: '発売日',
			value: item.date ? formatDate(item.date) : '情報なし',
			icon: '📅',
		},
		...
		{
			label: 'ジャンル',
			value: item.genre_names && item.genre_names.length > 0 ? item.genre_names : '情報なし',
			icon: '📚',
		},
		{
			label: 'メーカー',
			value:
				item.manufacture_names && item.manufacture_names.length > 0
					? item.manufacture_names
					: '情報なし',
			icon: '🏭',
		},
		{
			label: 'シリーズ',
			value: item.series_names && item.series_names.length > 0 ? item.series_names : '情報なし',
			icon: '📺',
		},
	]
	...
	const getLink = (label: string, item: { name: string }) => {
		switch (label) {
			case 'ジャンル':
				return `/fbooks/genre/${encodeURIComponent(item.name)}`
			case 'メーカー':
				return `/fbooks/maker/${encodeURIComponent(item.name)}`
			case 'シリーズ':
				return `/fbooks/series/${encodeURIComponent(item.name)}`
			default:
				return '#'
		}
	}
						...
						<TableCell className='p-4'>
							...
							Array.isArray(value) ? (
								...
								<Link
									...
									<h2>{Array.isArray(value) ? value.join(', ') : value}</h2>
								</Link>
							) : (
								<p className='text-base text-gray-900 dark:text-gray-100 break-words'>
									{Array.isArray(value) ? value.join(', ') : value || '情報なし'}
								</p>
							)}
						</TableCell>
					</TableRow>
...
// 			return parts.join(' ')
// 		})()
// 		const url = `https://erice.cloud/fbooks/itemd/${params.dbId}` // 実際のURLに置き換えてください
...
// アイテムデータの取得関数
async function fetchItemData(dbId: string): Promise<FbooksKobetuItem> {
	const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/fbooks-item?db_id=${dbId}`, {
...
export default async function DoujinKobetuItemPage({ params }: Props) {
	try {
		...
		const description = (() => {
			...
			if (item.date) {
				parts.push(`この同人作品の発売日は${formatDate(item.date)}。`)
			}

			if (item.manufacture_names && item.manufacture_names.length > 0) {
				parts.push(`メーカーは${item.manufacture_names[0]}から発売されています。`)
			}

			if (item.series_names && item.series_names.length > 0) {
				parts.push(`シリーズは${item.series_names[0]}です。`)
			}

			return parts.join(' ')
		})()

		const jsonLdData = generateFbooksKobetuItemStructuredData(item, description)
		const jsonLdString = JSON.stringify(jsonLdData)

		// パンくずリストデータの生成
		const breadcrumbData = generateFbooksBreadcrumbList(item)
					...
					<div className='container mx-auto px-4 sm:px-6 py-8 sm:py-12'>
						...
						<article className='bg-white dark:bg-gray-800  shadow-lg p-6 sm:p-8 space-y-8'>
							...
							{item.manufacture_names && item.manufacture_names.length > 0 && (
								<MakerTimelinePage
									searchParams={{
										maker_id: item.manufacture_names[0].toString(),
										maker_name: item.manufacture_names[0],
									}}
								/>
							)}
							{item.series_names && item.series_names.length > 0 && (
								<SeriesTimelinePage
									searchParams={{
										series_id: item.series_names[0].toString(),
										series_name: item.series_names[0],
									}}
								/>
							)}
