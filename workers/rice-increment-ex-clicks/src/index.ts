...
async function incrementPageClicks(db: D1Database, articleId: number): Promise<void> {
	const result = await db
		.prepare('UPDATE articles SET external_clicks = external_clicks + 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
		.bind(articleId)
		.run();
...
const worker = {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		try {
			...
			return new Response(JSON.stringify({ success: true, message: 'EX clicks incremented successfully' }), {
				status: 200,
				headers: { 'Content-Type': 'application/json' },
			});
		} catch (error) {
